        ...
        for m in range(len(RhoOfBandEdges)-1):
            ...
            for m2 in range(m+1, len(RhoOfBandEdges)):
                ...
                if l > BAND_WIDTH_MAX:
                    break
                bm = mask.BandMask()
                bm.setParam(bdata2.putTheta(), bdata2.putEdgeRanges(), ebsd.Circle)
...
def run():
    ...
    try:
        ...
        PC0 = ebsd.PC0         # 下式でproject centerの座標（3次元ベクトル, スケール変換前）は求められるとする:
        Circle = ebsd.Circle   # True: EBSD画像は円, False: 四角
